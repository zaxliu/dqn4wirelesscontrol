...
#=====================================================
    def step(self, last_ack):
        ...
        if last_ack is not None:
            temp = self.observation[-1]
            if temp[1] == 1:
                self.queue -= 1 if self.queue > 0 else 0
            if temp[1] == -1:
                self.queue = self.queue+1

        return self.translate_action_((0, 0) if action[1] == 1 and self.queue == 0 else action), reward
...
#======================================================
    def evaluate_ack_(self, ack):
        ...
        if ack is not None and (ack in self.routing_childs or ack == self.parent):
            return 1
        elif self.observation[-1] is not None and self.observation[-1][1] == 0:
            return 0
        else:
            return -0.1
        #
        ...
        #     return 0
        # else:
        #     if ack in self.routing_childs or ack == self.parent:
        #         return 1
        #     else:
        #         return 0
        #
