...
class TrafficServer:
    ...
    # Public Methods
    def feed_traffic(self, traffic_df):
        """Take in current traffic

        Dummy function to temporally register the traffic of current epoch
        :param traffic_df: traffic DataFrame
        :return: Nothing
        """
    ...
    def observe(self):
        """Compile observation for controller
        Observe from traffic and queue, then compile a summary as infomation.
        :return:
        """
        if self.traffic_df is None:
            print "Please feed_traffic first..."
            raise ValueError
        traffic_observation = self.__observe_traffic__()
        q_observation = self.__observe_q__()
        return traffic_observation, q_observation

    def get_service_and_cost(self, control):
        """Generate service based on the control commands from controller.
        :param control:
        :return:
        """

        sleep_flag, control_req = control  # extract control commands
        self.__enqueue_all_traffic__()  # put all incoming requests into queue
        ...
        # else...
            ...
            service = self.__serve_requests__(control_req)
            cost = 1  # set cost=1
        # Prepare for next epoch
        self.traffic_df = None  # clear traffic buffer
        self.epoch += 1
        return service, cost
    ...
    def __observe_traffic__(self):
        ...
        num_req = 0
        num_bytes = 0
        for idx in self.traffic_df.index:
            bytesSent_req_domain = json.loads(self.traffic_df.loc[idx, 'bytesSent_per_request_per_domain'])
    ...
    def __enqueue_all_traffic__(self):
        enq_df = self.traffic_df
        enq_df['arriveTime_epoch'] = self.epoch
        self.q = self.q.append(enq_df, ignore_index=True)
        return

    def __dequeue_all_traffic__(self):
        q = self.q
        self.q = pd.DataFrame(columns=['sessionID', 'uid', 'arriveTime_epoch' 'bytesSent_per_request_per_domain'])
        return q

    def __serve_requests__(self, control_req):
        ...
        # Serve queued requests ===============================================================
        # 1. dequeue and serve all queued requests
        if service_str == 'serve_all':
            q = self.__dequeue_all_traffic__()
            for idx in q.index:
                service_req_domain = {}
        ...
        # 3. randomly serve or keep queuing requests
            ...
            for idx in q.index:
                service_req_domain = {}
                sessionID = (int(sessionID) == q['sessionID']).nonzero()[0][0]
                bytesSent_req_domain = json.loads(q.loc[idx, 'bytesSent_per_request_per_domain'])
                for domain in bytesSent_req_domain:
                    for reqID in bytesSent_req_domain[domain]:
                        if domain not in service_req_domain:
                        ...
                        if r < 0.5:
                            service_req_domain[domain][reqID] = 'serve'
                            num_serve += 1
                            del bytesSent_req_domain[domain][reqID]
                        else:
                            service_req_domain[domain][reqID] = 'queue'
                            num_queue += 1
                    if len(bytesSent_req_domain[domain]) == 0:
                        del bytesSent_req_domain[domain]
                q.loc[idx, 'service_per_request_per_domain'] = json.dumps(service_req_domain)
                if len(bytesSent_req_domain) == 0:
                    self.q.drop([idx], axis=0, inplace=True)
                else:
